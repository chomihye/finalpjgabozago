...
public interface MypageReserService {		// 숙소예약내역 페이지를 위한 서비스 객체
	...
	public abstract List<LinkedHashMap<String, Object>> getUserReserList(Criteria cri, MemberVO member) throws ServiceException;
	
	
	// 예약내역 불러올 때, 예약상태 체크
	// 체크인날짜가 지났으면 DB에 예약상태를 CA -> UCRN로 변경
	// 체크아웃 날짜로부터 1년이 지났으면 DB에 예약상태를 UCRN -> UCRY로 변경
	public abstract void modifyReserStatus(MemberVO member) throws ServiceException;
	
	
	// 특정 숙소예약내역 detail 페이지 정보 불러오는 메소드
	public abstract Map<String, Object> getOneReserDetail(AccomReservationDTO reser, MemberVO member) throws ServiceException;
	...
	public abstract Map<String, Object> getAccomInfo(AccomReservationDTO reser, MemberVO member) throws ServiceException;
