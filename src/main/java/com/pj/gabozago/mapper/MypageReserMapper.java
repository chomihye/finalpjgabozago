...
public interface MypageReserMapper {		// 숙박예약내역 페이지 관련 mapper
	...
	public abstract List<LinkedHashMap<String, Object>> selectUserReserList(@Param("cri") Criteria cri, @Param("member") MemberDTO member) throws DAOException;
	
	// 예약내역 불러올 때, 체크인날짜가 지났으면 DB에 예약상태를 CA -> UCRN로 변경하는 메소드
	public abstract void updateStatusCaToUcrn(MemberDTO member) throws DAOException;
	
	// 예약내역 불러올 때, 체크아웃 날짜로부터 1년이 지났으면 DB에 예약상태를 UCRN -> UCRY로 변경하는 메소드
	public abstract void updateStatusUcrnToUcry(MemberDTO member) throws DAOException;
	
	// 특정 숙소예약내역 detail 페이지 정보 불러오는 메소드
	public abstract Map<String, Object> selectOneReserDetail(AccomReservationDTO reser) throws DAOException;
	...
	public abstract Map<String, Object> selectAccomInfo(AccomReservationDTO reser) throws DAOException;
